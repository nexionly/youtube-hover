---
name: hyperframes
description: Create video compositions, animations, title cards, overlays, captions, voiceovers, audio-reactive visuals, and scene transitions in HyperFrames HTML. Includes CLI commands (init, lint, preview, render, transcribe, tts), GSAP animation help, block registry management, and converting websites to video. Use when asked to build any HTML-based video content, capture a website for video, use hyperframes CLI, or help with GSAP animations in a video context. Covers composition authoring, timing, media, and the full video production workflow.
---
...
For specific tasks, refer to these specialized reference documents:
- **CLI Commands**: [references/cli.md](references/cli.md) (init, render, preview, etc.)
- **GSAP Animations**: [references/gsap.md](references/gsap.md) (timelines, easing, staggers)
- **Block Registry**: [references/registry.md](references/registry.md) (adding pre-built blocks)
- **Website to Video**: [references/url-to-video.md](references/url-to-video.md) (capturing URLs and promo creation)
